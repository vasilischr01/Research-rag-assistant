...
The test suite now covers retrieval behavior, API behavior, upload hardening, security headers, rate limiting, and sanitized error responses.
...
\- Hardened PDF upload validation
...
\| \`dense\_reranked\` | Dense → Cross-Encoder | Yes |

\| \`hybrid\_reranked\` | Dense + BM25 → RRF → Cross-Encoder | Yes |
...
Duplicate filenames return `409 Conflict`. Oversized PDFs return `413 Request Entity Too Large`.

The application also sanitizes client-facing processing errors instead of returning raw internal exception messages.
...
  "reranker\_score": null

}

\`\`\`

For a \`dense\_reranked\` result, the response may instead contain:
...
RR = 1 / rank\_of\_first\_relevant\_result
...
\- \`candidate\_k = 20\` produced the highest Recall\@5 for the evaluated reranking configuration.

\- Increasing the pool from 20 to 40 increased latency without improving retrieval quality.

\- \`candidate\_k = 5\` represented a strong low-latency reranking operating point.
...
evaluation/retrieval\_eval.json
...
evaluation/retrieval\_benchmark.json
...
evaluation/candidate\_k\_ablation.json
...
The FastAPI layer includes production-style safeguards around request handling and document ingestion.
...
Normal JSON/API requests are limited to `64 KiB`. Upload requests use a separate multipart limit, while the PDF payload itself is enforced at `20 MiB` during streaming.

**## Rate Limiting**

The current API uses a lightweight in-memory limiter:

```text
60 requests / 60 seconds / client IP
```

Requests over the limit return `429 Too Many Requests` with `Retry-After: 60`.

This is suitable for a local/demo single-instance deployment. A distributed deployment should move rate limiting to a shared store, ingress controller, or API gateway.

**## Error Sanitization**

Expected validation errors return controlled `400` responses. Unexpected internal errors return generic `500` responses without exposing internal exception strings to clients.
...
\|   |-- retrieval\_eval.json

\|   |-- retrieval\_benchmark.json

\|   \`-- candidate\_k\_ablation.json
...
The optional language-model key is read from `LLM_API_KEY` at runtime. The tracked `.env.example` contains an empty placeholder only; real credentials belong in the ignored `.env` file or an external secret-management system.
...
\- The current rate limiter is process-local and is not shared across replicas.

\- No authentication or RBAC layer is currently included.

\- PDF signature validation is lightweight and is not a malware scanner.
